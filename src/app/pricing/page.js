...
const PLANS = [
  ...
  },
  {
    ...
    credits: 600,
    description:
      "Designed for power users demanding batch exports and high speed.",
    popular: true,
  },
  {
    id: "business",
    ...
    credits: 2000,
    description:
      "Maximum value pack for agency workflows and large volume generations.",
  },
];
    ...
    try {
      ...
      toast.error(
        err.response?.data?.error ||
          "Failed to trigger Stripe checkout session.",
      );
    } finally {
      setLoadingPlan(null);
    }
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 py-12 sm:px-6 lg:px-8 flex flex-col gap-10 overflow-y-auto scrollbar-subtle items-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-xs sm:text-sm text-secondary-text max-w-lg leading-relaxed">
            Purchase flexible credit packages to perform high-resolution
            predictions. Keep all profits — we handle AI infrastructure.
          </p>
        </div>
          ...
          {PLANS.map((plan) => (
            <div
              ...
              className={`relative bg-bg-card border rounded-lg p-6 flex flex-col justify-between gap-6 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 ${
                plan.popular
                  ? "border-primary shadow-xl shadow-primary/5 scale-105"
